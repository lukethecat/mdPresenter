...
**Liquid Glass** —— macOS 26+ 上使用系统原生的 `glassEffect`（可着色、可交互，与 Apple 新版 iWork 套件同款）；macOS 11–25 自动降级为 `NSVisualEffectView` 模糊玻璃。缩略图、预览、检查器、状态栏、TurboStart 横幅、工具栏按钮与演讲者视图侧栏全部浮在玻璃上，背后是随当前幻灯片颜料色缓缓呼吸的环境光。
